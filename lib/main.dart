...
class AIService {
  ...
  static Stream<String> generateAIResponseStream(String prompt) async* {
    try {
      final content = [Content.text(prompt)];
  ...
  static Future<Map<String, Stream<String>>> generateComprehensiveAnalysis(Cryptocurrency crypto) async {
    String currentDate = DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now());
    
    return {
      'general': generateAIResponseStream(
        "Provide a comprehensive general analysis of ${crypto.name} (${crypto.symbol}) in the cryptocurrency market as of $currentDate. Include: ..."
        "1. Current price: \$${crypto.price.toStringAsFixed(2)} - compare this to 7-day and 30-day averages\n"
      "2. Market cap: \$${crypto.marketCap.toStringAsFixed(2)} - discuss its rank and recent changes\n"
      "3. 24-hour trading volume and how it compares to the 7-day average\n"
      ...
      'fundamental': generateAIResponseStream(
        "Analyze the current fundamental aspects of ${crypto.name} (${crypto.symbol}) as of $currentDate. Cover: ..."
         "1. Latest technological updates or changes to the protocol\n"
      "2. Most recent partnerships or adoption metrics\n"
      "3. Upcoming events or releases in the next 30 days\n"
...
class _CryptocurrencyDetailScreenState extends State<CryptocurrencyDetailScreen> with SingleTickerProviderStateMixin {
  ...
  List<FlSpot> priceData = [];
  int selectedChartDays = 7;
  Map<String, StreamController<String>> aiAnalysisControllers = {
    'general': StreamController<String>(),
    'news': StreamController<String>(),
    'fundamental': StreamController<String>(),
    'team': StreamController<String>(),
  };
  String aiQuestion = '';
  String aiAnswer = '';
late StreamController<String> aiQuestionController;
  @override
  void initState() {
    super.initState();
    ...
    fetchDetails();
    fetchComprehensiveAnalysis();
    aiQuestionController = StreamController<String>();
  }
   void _initializeStreamControllers() {
    aiAnalysisControllers = {
      'general': StreamController<String>.broadcast(),
      'news': StreamController<String>.broadcast(),
      'fundamental': StreamController<String>.broadcast(),
      'team': StreamController<String>.broadcast(),
    };
  }
 @override
  void dispose() {
    aiAnalysisControllers.forEach((_, controller) => controller.close());
  //  aiQuestionController.close();
    super.dispose();
  }
  ...
  Future<void> fetchComprehensiveAnalysis() async {
    try {
      final analysisStreams = await AIService.generateComprehensiveAnalysis(widget.cryptocurrency);
      analysisStreams.forEach((key, stream) {
        aiAnalysisControllers[key]?.addStream(stream);
      });
    } catch (e) {
      _showErrorSnackbar('Failed to fetch AI analysis');
    }
   ...
   Future<void> askAIQuestion() async {
    if (aiQuestion.isEmpty) return;
    aiQuestionController.add('Loading...');
    try {
      final stream = AIService.generateAIResponseStream(
        "Answer this question about ${widget.cryptocurrency.name} (${widget.cryptocurrency.symbol}) as of ${DateFormat('yyyy-MM-dd HH:mm:ss').format(DateTime.now())}: $aiQuestion"
      );
      aiQuestionController.addStream(stream);
    } catch (e) {
      aiQuestionController.add('Failed to get AI response');
    }
  }
...
Widget _buildLoadingScreen() {
  ...
  Widget _buildAIInsights() {
    return Column(
      ...
      children: [
        const Text('AI Insights', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
        const SizedBox(height: 16),
        _buildAIInsightCard('General Analysis', aiAnalysisControllers['general']!.stream),
        _buildAIInsightCard('Latest News', aiAnalysisControllers['news']!.stream),
        _buildAIInsightCard('Fundamental Analysis', aiAnalysisControllers['fundamental']!.stream),
        _buildAIInsightCard('Team Details', aiAnalysisControllers['team']!.stream),
        const SizedBox(height: 16),
        ElevatedButton(
          onPressed: fetchComprehensiveAnalysis,
          child: const Text('Refresh AI Insights'),
 ...
 Widget _buildAIInsightCard(String title, Stream<String> contentStream) {
    return Card(
      elevation: 2,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
      margin: const EdgeInsets.only(bottom: 16),
            ...
            child: StreamBuilder<String>(
              ...
              builder: (context, snapshot) {
                if (snapshot.connectionState == ConnectionState.waiting) {
                  ...
                  return MarkdownBody(
                    data: snapshot.data ?? 'No data available',
                    styleSheet: MarkdownStyleSheet(
                      p: TextStyle(fontSize: 14),
                      h1: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
                      h2: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
    ...
    return ListView(
      ...
      children: [
        ...
        StreamBuilder<String>(
          stream: aiQuestionController.stream,
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const CircularProgressIndicator();
            } else if (snapshot.hasError) {
              ...
              return MarkdownBody(
                data: snapshot.data ?? 'No answer yet.',
                styleSheet: MarkdownStyleSheet(
                  p: TextStyle(fontSize: 14),
                  h1: TextStyle(fontSize: 20, fontWeight: FontWeight.bold),
                  h2: TextStyle(fontSize: 18, fontWeight: FontWeight.bold),
